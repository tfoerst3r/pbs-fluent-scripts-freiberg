...
# Description : chain script for Fluent on the new HPC cluster Freiberg
# Required    :    
...
if [ $calctime -eq 12 ]; then
  echo 'New process will start in 11h 40min'
elif [ $calctime -eq 24 ]; then
  echo 'New process will start in 23h 40min'
elif [ $calctime -eq 36 ]; then
  echo 'New process will start in 35h 40min'
elif [ $calctime -eq 168 ]; then
  echo 'New process will start in 167h 40min'
elif [ $calctime -eq 1 ]; then
  echo 'New process will start in 0h 50min'
else 
  echo 'Is your time input correct?'
  echo 'New process will start in 23h 40min'
fi


bash_fluent="#!/bin/bash

#PBS -N $jobname
#PBS -l select=$Ichunk:ncpus=$IinChunk:mem=$memstring:mpiprocs=$IinChunk:ngpus=$gpu 
...
#-- generation of exit trigger --#

if [ $calctime -eq 12 ]; then
  sleep 11h 40m && touch $exit_file &
elif [ $calctime -eq 24 ]; then
  sleep 23h 40m && touch $exit_file &
elif [ $calctime -eq 1 ]; then
  sleep 0h 50m && touch $exit_file &
elif [ $calctime -eq 36 ]; then
  sleep 35h 40m && touch $exit_file &
elif [ $calctime -eq 168 ]; then
  sleep 167h 40m && touch $exit_file &
else 
  sleep 23h 40m && touch $exit_file &
fi

fluent $Dim\"ddp\" -t $Itot -gpgpu=0 -g -mpi=intel -cnf=\$PBS_NODEFILE -i $journal > $outfile
" 

#---------------------#
# generate calc files #
#---------------------#
echo "$bash_fluent"    > $run
